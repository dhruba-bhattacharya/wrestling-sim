...
const imageDetailInput = document.getElementById("imageDetailInput");
const imageDetailValue = document.getElementById("imageDetailValue");
...
function updateImageDetailReadout() {
  const value = Number(imageDetailInput.value);
  imageDetailValue.textContent = `${value} / 10`;
}
...
function drawSprite(traits) {
  ...
  if (traits.imagePattern) {
    drawImageResemblanceSprite(traits, px);
  } else {
  ...
  applyComplexityDetails(traits, px);
  renderTraitSummary(traits);
  updateComplexityMeter(traits);
}

function drawImageResemblanceSprite(traits, px) {
  const pattern = traits.imagePattern;
  const drawRegion = (dx, dy, dw, dh, syStart, syEnd) => {
    for (let y = 0; y < dh; y++) {
      const v = syStart + ((y + 0.5) / dh) * (syEnd - syStart);
      for (let x = 0; x < dw; x++) {
        const u = (x + 0.5) / dw;
        px(dx + x, dy + y, 1, 1, samplePattern(pattern, u, v));
      }
    }
  };

  drawRegion(8, 2, 8, 10, 0.0, 0.25);
  drawRegion(7, 12, 10, 15, 0.25, 0.7);
  drawRegion(8, 27, 8, 12, 0.7, 1.0);

  drawRegion(5, 14, 3, 12, 0.3, 0.75);
  drawRegion(16, 14, 3, 12, 0.3, 0.75);

  px(7, 12, 10, 1, traits.skin);
  px(9, 37, 3, 2, "#f8f8f8");
  px(12, 37, 3, 2, "#f8f8f8");

  if (traits.mask !== "none") {
    px(9, 8, 6, 3, traits.accentColor);
  }
}

function samplePattern(pattern, u, v) {
  const x = Math.max(0, Math.min(pattern.width - 1, Math.floor(u * pattern.width)));
  const y = Math.max(0, Math.min(pattern.height - 1, Math.floor(v * pattern.height)));
  return pattern.colors[y * pattern.width + x];
}

function applyComplexityDetails(traits, px) {
  ...
  if (traits.complexity >= 7) {
    px(10, 8, 1, 1, "#ffffff");
    px(13, 8, 1, 1, "#ffffff");
    px(9, 36, 3, 1, traits.accentColor);
    px(12, 36, 3, 1, traits.accentColor);
  }
  if (traits.complexity >= 9) {
    px(6, 21, 2, 1, traits.gearColor);
    px(16, 21, 2, 1, traits.gearColor);
    px(11, 21, 2, 1, "#ffffff");
    px(10, 26, 4, 1, traits.accentColor);
  }
}
...
function extractImagePattern(image, detailLevel) {
  const width = 14 + detailLevel;
  const height = 22 + detailLevel * 2;
  ...
  for (let i = 0; i < data.length; i += 4) {
    colors.push(rgbaToHex(data[i], data[i + 1], data[i + 2]));
  }
...
function traitsFromImage() {
  ...
  const accentGuess = averageColor(data, 0, 40, 64, 18);

  const imageDetail = Number(imageDetailInput.value);
  ...
  return {
    ...
    accentColor: nearestColor(accentGuess, OPTIONS.accentColor),
    complexity: Math.max(imageDetail, 4),
    source: "Image",
    imagePattern: extractImagePattern(uploadedImage, imageDetail)
  };
}
...
imageDetailInput.addEventListener("input", updateImageDetailReadout);

fillPreferenceSelectors();
updateComplexityReadout();
updateImageDetailReadout();
drawSprite(randomTraitsFromPreferences());
